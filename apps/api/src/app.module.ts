...
@Module({
  imports: [
    ...
    JwtModule.register({ global: true, secret: process.env.JWT_ACCESS_SECRET ?? "local-demo-secret-change-before-sharing", signOptions: { expiresIn: "15m" } }),
  ],
  controllers: [AuthController, HealthController, CountriesController, ScansController, RecordsController, SubscriptionController],
  providers: [MemoryStore, RuleEngine, AuthGuard],
})
